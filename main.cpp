...
void create_deque() {
    tpie::deque<Item> d = tpie::deque<Item>();

    int count = 0;
    while (count < 200000){
        d.pushFront(Item{1,1,count});
        count++;
    }
    ...
    while (!d.empty()){
        item = d.popBack();
        tpie::log_info() << "Deque pop: " << item.a << " " << item.b << " " << item.c << std::endl;
    }

    d.pushFront(Item{2,5,123});
    item = d.popBack();
    tpie::log_info() << "Deque pop: " << item.a << " " << item.b << " " << item.c << std::endl;
}
